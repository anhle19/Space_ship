...
class Laser:
    ...
    #Process collision between 2 ship or laser and ship
...
class Player(Ship):
    ...
    #draw healthbar anh health lost
...
class Enemy(Ship):
    ...
    #Enemy auto shot player
    def shot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x-20, self.y, self.laser_img)
            self.lasers.append(laser)
            self.cool_down_counter = 1 #check
...
def main():
    ...
    #Turn off vietnamese before play game
    while run:
        clock.tick(FPS)
        redraw_window()
        ...
        if keys[pygame.K_s] and player.y + player_vel + player.get_height() + 15< HEIGHT:#move down
            player.y += player_vel
        if keys[pygame.K_SPACE]:#shot laser
            player.shot()
        if keys[pygame.K_ESCAPE]:#end game
            main_menu()
